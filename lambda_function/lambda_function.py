...
def uploadFileToS3(filePath):
    ...
    client.upload_file(filePath, S3_BUCKET_NAME, filePath.replace('/tmp', 'results'))

def setRequestStatusInS3(requestId, requestLog, currentStatus):
    client = boto3.client('s3')
    client.put_object(
        Body=requestLog + f'> {currentStatus}',
        Bucket=S3_BUCKET_NAME,
        Key=f'request_status/{requestId}.txt',
        ContentType='text/plain'
    )
#===========================================================
def generateTopKeywords(keyword, jsonDump):
    map = {}
    ...
    filePath = f'/tmp/{keyword}/topKeywords.json'
...
def plotHelper(keyword, plotBy, countData):
    ...
    keywordCleaned = "_".join(keyword.lower().split())
    filePath = f'/tmp/{keywordCleaned}/{plotBy}Plot.png'
...
def generateAllPlots(keyword, jsonDump):
    plotYearWise(keyword, jsonDump)
    plotFieldWise(keyword, jsonDump, 'author')
    plotFieldWise(keyword, jsonDump, 'category')
#===========================================================
def generateAllPapersFile(keyword, jsonDump):
    filePath = f'/tmp/{keyword}/allPapers.json'
    with open(filePath, mode='w', encoding='utf-8') as jsonFile:
        json.dump(jsonDump, jsonFile, indent=2, ensure_ascii=False)
    uploadFileToS3(filePath)
#===========================================================
def startRequest(keyword, requestId):
    ...
    requestLog = ''
    setRequestStatusInS3(requestId, requestLog, PROCESSING)
    ...
    while batch <= 100:
        try:
            ...
            if 'entry' in resp:
                ...
                requestLog += f"> batch-{batch}: added {len(curBatch)} CS papers\n"
            else:
                break
        except Exception as ex:
            requestLog += f"> error while processing batch-{batch}: {ex}\n"
            setRequestStatusInS3(requestId, requestLog, ERROR)
            return
        batch += 1
    ...
    keywordCleaned = "_".join(keyword.lower().split())
    os.mkdir(f"/tmp/{keywordCleaned}")
    try:
        generateTopKeywords(keywordCleaned, outLs)
        requestLog += f"> generated top 100 keywords\n"
        setRequestStatusInS3(requestId, requestLog, PROCESSING)
    except Exception as ex:
            requestLog += f"> error while generating top keywords: {ex}\n"
            setRequestStatusInS3(requestId, requestLog, ERROR)
            return

    try:
        generateAllPlots(keyword, outLs)
        requestLog += f"> generated bar charts for count of papers by year, author, category\n"
        setRequestStatusInS3(requestId, requestLog, PROCESSING)
    except Exception as ex:
            requestLog += f"> error while generating count bar charts: {ex}\n"
            setRequestStatusInS3(requestId, requestLog, ERROR)
            return
        
    try:
        generateAllPapersFile(keywordCleaned, outLs)
        requestLog += f"> generated json file containing all papers\n"
        setRequestStatusInS3(requestId, requestLog, PROCESSING)
    except Exception as ex:
            requestLog += f"> error while generating all papers json file: {ex}\n"
            setRequestStatusInS3(requestId, requestLog, ERROR)
            return
    
    requestLog += f"> processed total of {len(outLs)} '{keyword}' papers\n"
    setRequestStatusInS3(requestId, requestLog, SUCCESS)

    client = boto3.client('s3')
    client.put_object(
        Body=datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ'),
        Bucket=S3_BUCKET_NAME,
        Key=f'results/{keywordCleaned}/lastProcessed.txt',
        ContentType='text/plain'
    )
...
def lambda_handler(event, context):
    ...
    requestId = event['context']['request-id']
    startRequest(keyword, requestId)
